...
const CourseTypesContainer = () => {
  ...
  const fetchList = async () => {
    try {
      ...
      const { data } = await request.post(URLS.course_types_filter, payload);
      setList({ content: get(data,'content',[]), totalElements: get(data,'totalElements',0), totalPages: get(data,'totalPages',0) });
    } catch {
      notification.error({ message: 'Failed to load course types' });
    } finally { setLoading(false); }
  };
  useEffect(()=>{ fetchList(); }, [page, JSON.stringify(filters)]);

  const onActivate = async (record, activate) => {
    try { await request.patch(URLS.course_types_activate(get(record,'id'), activate)); fetchList(); }
    catch { notification.error({ message: 'Failed to update' }); }
  };

  const onDelete = async (id) => { try { await request.delete(URLS.course_types_delete(id)); fetchList(); } catch { notification.error({message:'Failed to delete'}) } };

  const onFinish = async (values) => {
    try {
      setSaving(true);
      if (selected) { await request.patch(URLS.course_types_patch(get(selected,'id')), values); }
      else { await request.post(URLS.course_types_create, values); }
...
export default CourseTypesContainer;
